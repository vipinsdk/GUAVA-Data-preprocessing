    ...
    for nf in tqdm(range(start, end), desc='render'):
        ...
        if args.vis_smpl: 
            ...
            if args.mesh_root is None:
                ...
                mesh_path_0 = os.path.join(args.mesh_root, '{:06d}_0.obj'.format(nf))
                mesh_path_1 = os.path.join(args.mesh_root, '{:06d}_1.obj'.format(nf))
                print(mesh_path_0, mesh_path_1)
                if os.path.exists(mesh_path_0) and os.path.exists(mesh_path_1):
                    mesh_0 = trimesh.load(mesh_path_0)
                    mesh_1 = trimesh.load(mesh_path_1)
                    combined_vertices = np.vstack((mesh_0.vertices, mesh_1.vertices))
                    combined_faces = np.vstack((mesh_0.faces, mesh_1.faces + len(mesh_0.vertices)))
                    dataset.vis_smpl(vertices=combined_vertices, faces=combined_faces, images=images, nf=nf, sub_vis=args.sub_vis, add_back=True)

                    mesh_folder = join(args.out, 'mesh')
                    os.makedirs(mesh_folder, exist_ok=True)
                    trimesh.Trimesh(faces=combined_faces, vertices=combined_vertices, process=False).export(f'{mesh_folder}/{nf}.obj')
        if args.vis_repro:
